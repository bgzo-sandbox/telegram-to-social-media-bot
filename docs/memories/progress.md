...
- [x] 2026-02-19: 已按 shell 方式执行旧单文件迁移到待删除目录（不再直接删除）。
    - [x] 执行后 `archives/person` 根目录旧单文件数量：0；
    - [x] 执行后 `archives/channel` 根目录旧单文件数量：0；
    - [x] `archives/260218-legacy-pending-delete` 当前累计文件：154。

- [x] 2026-02-19: 已完成“负责人签字后正式迁移演练”，并更新最终切换结论为 Go。
    - [x] 发布前检查记录已更新：审批门禁=通过；
    - [x] 一次性切换结论已从 No-Go 更新为 Go；
    - [x] 风险说明已更新为“无阻断项”。
...
- [x] 2026-02-19: 已完成端到端迁移联调与 Go/No-Go 记录（步骤2+3+4）。
    - [x] 新增 `RunMigrationDrill()`，串联步骤3补齐与步骤4备份后迁移到待删除目录；
    - [x] 新增门禁判定输出：功能/数据/完整性/运维/审批；
    - [x] 已补联调单测：`Go`（审批通过）与 `No-Go`（审批未通过）两条路径；
    - [x] 已在发布前检查文档补充 2026-02-19 联调与 Go/No-Go 结果记录；
...
- [ ] 结合真实负责人签字执行一次正式迁移演练，更新最终 Go 结论并准备发布。

- [x] 2026-02-19: 代码阶段步骤4完成（旧单文件备份并迁移到待删除目录，先备份后移动）。
    - [x] 新增 `BackupAndMoveLegacySingleFiles()`：扫描 `person/channel` 根目录下旧单文件 `source_id.md`；
    - [x] 移动前强制生成备份 `archives/260218-old-markdown-archives.zip`（含 `person/*.md`、`channel/*.md` 条目）；
    - [x] 旧单文件统一移动到 `archives/260218-legacy-pending-delete`，不影响新结构目录 `source_id/message_id.md`；
    - [x] 新增单元测试：备份+移动成功、无旧文件时跳过；
    - [x] 验证通过：`go test ./internal/service/archivemigrationservice ./internal/service/archiveservice ./internal/service/pipelineservice ./internal/Database`。
