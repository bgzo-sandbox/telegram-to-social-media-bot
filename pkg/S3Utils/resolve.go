...
// BuildR2ObjectKey 构造 R2 对象键：<path>/<source_id>/<message_id>/<filename>。
// 这样做的原因是把 key 命名规则收口在单点，保证实时上传与历史迁移生成一致的键。
// 每段先 TrimSpace 再 Trim "/"，空段跳过，最终键不以 "/" 开头或结尾。
func BuildR2ObjectKey(pathPrefix string, sourceID string, messageID int64, filename string) string {
	segments := []string{
